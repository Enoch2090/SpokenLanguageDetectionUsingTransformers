...
lr=1e-5
epoch=25
tr_data=/root/autodl-tmp/ast/egs/audioset/data/datafiles/train_data.json

te_data=/root/autodl-tmp/ast/egs/audioset/data/datafiles/eval_data.json
freqm=48
timem=192
mixup=0.5
...
if [ -d $exp_dir ]; then
  echo 'exp exist, deleting current'
  rm -r exp
fi
mkdir -p $exp_dir
...
--label-csv ./data/class_labels_indices.csv --n_class 5 \
