...
if args.model == 'ast':
    ...
    audio_conf = {'num_mel_bins': 128, 'target_length': 512, 'freqm': 24, 'timem': 192, 
                  'mixup': 0.5, 'skip_norm': True, 'mode': 'train', 'dataset': 'audioset'}
    
    # integrate get_norm_states.py
    train_loader = torch.utils.data.DataLoader(
        dataloader.AudiosetDataset(os.path.join(os.getcwd(), 'data/datafiles/train_data.json'), 
                                   label_csv=os.path.join(os.getcwd(), 'data/class_labels_indices.csv'),
    ...
    norm_stats = {'audioset':[np.mean(mean), np.mean(std)], 'esc50':[-6.6268077, 5.358466], 'speechcommands':[-6.845978, 5.5654526]}
    target_length = {'audioset':512, 'esc50':512, 'speechcommands':128}
    ...
    if args.bal == 'bal':
        ...
        path = os.path.join(os.getcwd(),'data/datafiles/train_data_weight.csv')
        #samples_weight = np.loadtxt(args.data_train[0:-5]+'_weight.csv', delimiter=',')
        samples_weight = np.loadtxt(path, delimiter=',')
